...
newsr_goldset = _newsr.groupby('question_id').apply(
    lambda x: x.sample(n=3, replace=False, random_state=42) 
    if len(x[x['title'].notna() & x['abstract'].notna()]) >= 3 
    else x[x['title'].notna() & x['abstract'].notna()].sample(
        n=min(3, len(x[x['title'].notna() & x['abstract'].notna()])), 
        replace=False, 
        random_state=42
    )
